    ...
    def __init__(
        ...
        **kwargs,
        ...
        if train_collator is None and eval_collator is None and test_collator is None:
            raise ValueError(
                "use different collator for trainer but get no collator function."
            )
        if (
            eval_collator is not None
            and test_collator is not None
            and eval_collator != test_collator
        ):
            warnings.warn(
                "[WARNING!!!] use different collator for eval and test. but maybe do_eval and "
        ...
        self._eval_collator = (
            eval_collator if eval_collator is not None else self._train_collator
        )
        self._test_collator = (
            test_collator if test_collator is not None else self._eval_collator
        )
        if "data_collator" in kwargs and kwargs["data_collator"] is not None:
            warnings.warn(
                "use different collator for trainer but get 'data_collator' argument. It will take no effect and be ignored."
            )
...
class TrainerForMMLLM(TrainerDifferentCollatorMixin, Seq2SeqTrainer):
    ...
    def _prepare_input(
        ...
        if isinstance(data, Mapping):
            ...
            # of the model while eval.
                ...
                and self.args.fp16_full_eval
                and (torch.is_floating_point(data) or torch.is_complex(data))
            ):
    ...
    def save_prediction(self, predict_results, file_key_prefix="predict"):
        ...
        np.save(
            os.path.join(self.args.output_dir, f"{file_key_prefix}_predictions.npy"),
            predict_results.predictions,
        )
        np.save(
            os.path.join(self.args.output_dir, f"{file_key_prefix}_label_ids.npy"),
        ...
        with open(
            os.path.join(
                self.args.output_dir, f"{file_key_prefix}_extra_prediction.jsonl"
            ),
            "a",
            encoding="utf-8",
        ) as g:
            for p, t, pi, ti in tqdm(
                ...
                total=len(preds),
                desc=f"saving prediction for {file_key_prefix}",
            ):
    ...
    def plot_loss(self) -> None:
        ...
        data = json.load(
            open(os.path.join(training_args.output_dir, TRAINER_STATE_NAME), "r")
        )
        ...
        plt.savefig(
            os.path.join(training_args.output_dir, FIGURE_NAME),
            format="png",
            transparent=True,
            dpi=300,
        )
...
class Seq2SeqDataCollator(DataCollatorForSeq2Seq):
    ...
    def __call__(
        self, features: Sequence[Dict[str, Sequence]], return_tensors=None
    ) -> Dict[str, torch.Tensor]:
        # evaluation/inference adopts left-padding while training adopts right-padding
        text_features = [
            {k: feature[k] for k in self.text_keys if k in feature}
...
class Seq2Seq2DataCollatorWithImage(Seq2SeqDataCollator):
    ...
    def truncate_features(self, features):
        ...
        if self.max_length is not None:
            ...
            for i in range(len(features)):
                feature = features[i]
                cur_seq_length = len(feature["input_ids"])
                if cur_seq_length > max_length:
                    if self.inference_mode:
                        feature = {
                            k: feature[k][cur_seq_length - max_length :]
    ...
    def __call__(
        ...
        ret = super().__call__(self.truncate_features(features), return_tensors)
