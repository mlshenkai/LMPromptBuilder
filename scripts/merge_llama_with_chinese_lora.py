...
parser.add_argument('--base_model', default="/code-online/shenkai/LLMTuning/sk_finetuning_7b",
                    type=str, help="Please specify a base_model")
parser.add_argument('--lora_model', default="/code-online/shenkai/LLMTuning/sk_finetuning_7b_alpaca/lora_model",
...
parser.add_argument('--output_dir', default='/code-online/shenkai/LLMTuning/sk_finetuning_7b_alpaca', type=str)
...
if __name__=='__main__':
    ...
    for lora_index, lora_model_path in enumerate(lora_model_paths):
        ...
        if hasattr(peft.LoraModel,'merge_and_unload'):
            ...
            model_to_save_model = [k for k in non_lora_keys if "modules_to_save.default" in k]
            for k in non_lora_keys:
                if k in model_to_save_model or "embed_tokens" not in k or "lm_head" not in k:
                    continue
                print(f"merging {k}")
                original_k = k.replace('base_model.model.','')
                base_model_sd[original_k].copy_(lora_model_sd[k])

            for k in model_to_save_model:
                print(f"merging {k}")
                original_k = k.replace("base_model.model.", "").replace("modules_to_save.default.", "")
                base_model_sd[original_k].copy_(lora_model_sd[k])

            for k in lora_keys:
                print(f"merging {k}")
                original_key = k.replace('.lora_A','').replace('base_model.model.','')
                if "default" in original_key:
                    original_key = original_key.replace("default.", "")
                assert original_key in base_model_sd, f"{original_key} not in base_mode_sd"
                lora_a_key = k
